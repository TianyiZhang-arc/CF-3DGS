...
def report_pose_error(w2c: torch.Tensor, w2c_gt: torch.Tensor, auc_idx=[1, 3, 5, 10, 20], small_system=True):
    # report auc
    ...
    # report ape
    w2c_aligned = align_ate_c2b_use_a2b(w2c.inverse(), w2c_gt.inverse(), w2c.inverse(), small_system).inverse()
    ape_rot, ape_trans = evaluate_camera_alignment(w2c_aligned[:, :3, :4], w2c_gt[:, :3, :4])
    auc_dict = {}
    for i in range(len(auc_idx)):
        auc_dict[auc_idx[i]] = auc[i]
    return auc_dict, ape_rot.mean().item(), ape_trans.mean().item()    

def print_pose_error(auc_dict, ape_rot, ape_trans):
    auc_print = 'AUC ['
    for k in auc_dict:
