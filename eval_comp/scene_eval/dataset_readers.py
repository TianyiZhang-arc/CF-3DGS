...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, cam_extrinsics_gt=None, cam_intrinsics_gt=None):
    """
    Args:
        cam_extrinsics: Dict{key: Image}
        cam_intrinsics: Dict{camera_id: Camera}
        cam_extrinsics_gt: Dict{name: Image}
        cam_intrinsics_gt: Dict{camera_id: Camera}
    """
    cam_infos = []
    for idx, key in enumerate(cam_extrinsics):
        ...
        if cam_extrinsics_gt is not None:
            R_gt = qvec2rotmat(cam_extrinsics_gt[extr.name].qvec)
            T_gt = np.array(cam_extrinsics_gt[extr.name].tvec)
        else:
            R_gt, T_gt = None, None
        ...
        cam_info = CameraInfo(uid=uid, R=R, T=T, R_gt=R_gt, T_gt=T_gt, FovY=FovY, FovX=FovX, K=K, image=image,
...
def create_random_points3d_from_tracks(tracks, id2image, use_track_color=True):
    print('building random points3d')
    N = len(tracks)
    pts = 2 * np.random.rand(N, 3) - 1 # (-1, 1)
    H, W = id2image[next(iter(id2image))].shape[:2]
    if use_track_color:
        colors = np.zeros((N, 3))
        for i, track in enumerate(tracks):
            c = []
            for (img_id, obs) in track:
                x, y = int(obs[0]), int(obs[1])
                if y >= 0 and y < H and x >= 0 and x < W:
                    c.append(id2image[img_id][y, x])
            # c = [id2image[img_id][y, x] for (img_id, obs) in track]
            colors[i] = np.mean(c, axis=0)
