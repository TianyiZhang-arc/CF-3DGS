...
from .lie_group_helper import convert3x4_4x4, inv_3x4
from .comp_ate import rotation_distance, translation_distance, evaluate_camera_alignment
...
def align_ate_c2b_use_a2b(traj_a, traj_b, traj_c=None):
    ...
    if not small_system:
        ...
        traj_c_aligned = torch.from_numpy(traj_c_aligned).float().to(device)
...
def prealign_w2c_small_camera_systems(pose_w2c: torch.Tensor, pose_GT_w2c: torch.Tensor): # TODO: reformat
    ...
    if n_first_fixed_poses > 1:
        ...
        for pair_id_0 in range(B):  # to avoid that it is too long
            for pair_id_1 in range(B):
                ...
                error_R_list.append(R_error)
                error_t_list.append(t_error)
                full_error.append(t_error * (R_error))
